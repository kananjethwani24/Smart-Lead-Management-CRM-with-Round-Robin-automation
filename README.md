...
🗺️ State-Based Assignment — Lead from Maharashtra → finds callers with Maharashtra in their assigned_states
📊 Daily Cap Enforcement — If a caller has hit their daily_lead_limit, they are skipped
🔄 Round Robin Rotation — Uses round_robin_state to remember who was last assigned, ensuring fair distribution
🌐 Global Fallback — If no state-specific callers exist, all available callers participate in the rotation
...
A new row is added (or any row is updated) in the Google Sheet
n8n's Google Sheets Trigger detects the change (anyUpdate event)
n8n sends an HTTP POST request to the ngrok public URL → /api/webhook/new-lead
The Next.js API receives the payload, runs the Round Robin assignment logic, and saves the lead to SQLite
The UI dashboard polls the API every few seconds and displays the new lead
...
Paste the code from google-apps-script.js in this repo
Set the WEBHOOK_URL constant to your ngrok URL + /api/webhook/new-lead
Run createTrigger() once to activate an on-edit trigger
Setup Instructions
Prerequisites
Node.js 18+
...
An n8n account (free at n8n.cloud)
...
Paste the entire JSON as a single line into GOOGLE_SERVICE_ACCOUNT_KEY
Step 3: Run the App
...
Go to n8n.cloud and create a new workflow
Add a Google Sheets Trigger node → connect your Google account → select your sheet → set event to Any Update
...
Activate the workflow (toggle Published)
Step 5: Add Callers
Open the CRM at http://localhost:3000
Go to Callers page → Add your sales team members
Set their assigned states and daily lead limits
Now every new lead from Google Sheets will automatically be assigned!
What I Would Improve With More Time
🔌 WebSocket / Server-Sent Events instead of polling — true real-time updates via Supabase Realtime or Socket.io
📋 Lead Detail View — click on a lead to see activity timeline, notes, follow-up history
📊 Analytics Dashboard — charts for leads per day, caller performance, conversion rates
🔐 Authentication — role-based access control (admin vs caller view) using NextAuth.js
🔄 Bulk Reassignment — when a caller goes unavailable, redistribute their leads automatically
🏷️ Lead Status Pipeline — track stages: New → Contacted → Qualified → Converted → Lost
📄 Pagination — handle large datasets (1000+ leads) without performance issues
🛡️ Rate Limiting — protect webhook endpoints from abuse using middleware
☁️ Cloud Deployment — deploy on Vercel with a proper cloud database (PlanetScale, Supabase, or Railway)
🧪 Automated Tests — unit tests for Round Robin edge cases, integration tests for API routes
